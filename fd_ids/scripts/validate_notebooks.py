#!/usr/bin/env python
"""Static gate on the generated notebooks. Cheap checks that each cost a Kaggle version."""
...
CFG_LATE_KEYS = {"data_id", "n_test", "content_id", "backend", "startup_seconds"}
...
NB = ROOT / "papers/fd-ids-2025/notebook"
...
targets = [(K, p) for K in (20, 50, 100) for p in (False, True)
           if (NB / (f"{K}c_probe" if p else f"{K}c")).is_dir()]

for K, probe in targets:
    d = NB / (f"{K}c_probe" if probe else f"{K}c")
    ...
    cfg = next((c for c in (cfg_keywords(x) for x in src if x.startswith("CFG = dict(")) if c),
               {})
    for key, val in (("lr", 0.001), ("mu", 0.01), ("lam", 0.5), ("beta", 0.1),
                     ("temperature", 3.0), ("rounds", 2 if probe else 50),
                     ("local_epochs", 1),
                     ("clip", 1.0), ("seed", 42), ("num_classes", 16), ("n_features", 66),
                     ("world_size", 2), ("n_clients", K),
    ...
    chk(re.fullmatch(rf"fdids_{K}c(_probe|_[a-z0-9]+)?", name) is not None,
        f"run_name {name!r} is not fdids_{K}c with an optional tag")
...
print("\n".join(f"  FAIL {f}" for f in fails) if fails else
      "  all checks passed for " + ", ".join(
          f"{K}c" + ("-probe" if p else "") for K, p in targets))
