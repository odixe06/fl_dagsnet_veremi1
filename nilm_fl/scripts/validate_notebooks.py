#!/usr/bin/env python
"""Static gate on the generated NILM-FL notebooks. Cheap checks that each cost a Kaggle version."""
...
NB = ROOT / "papers/nilm-li-2024/notebook"
...
for K, probe, dirname in targets:
    ...
    chk("build_model=build_model" in flow and "expect_params=N_PARAMS" in flow,
        "the verifier must rebuild the proxy and every w_s")
    ...
    chk(re.fullmatch(rf"nilm_{K}c(_probe|_[a-z0-9]+)?", name) is not None,
        f"run_name {name!r} is not nilm_{K}c with an optional tag")
