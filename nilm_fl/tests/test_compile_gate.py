...
def gate(wrapper, dropout=0.1):
    ...
    Se, Re = build_model(cfg), build_model(cfg)
    xb = torch.randn(B, 66) * 2; yb = torch.randint(0, 16, (B,))
    real = torch.compile
    torch.compile = lambda m, **kw: wrapper(m)
    try:
        Sc, Rc = driver._compile_train(Se, Re, cfg, torch.device("cpu"), xb, yb)
    finally:
        torch.compile = real
    ps = [m.p for m in list(Se.modules()) + list(Re.modules()) if isinstance(m, torch.nn.Dropout)]
    trainable = Se.training and Re.training and all(
        p.requires_grad for p in list(Se.parameters()) + list(Re.parameters()))
    return (Sc is not Se) and (Rc is not Re), ps, trainable
