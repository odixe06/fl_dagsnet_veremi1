...
proxy on the last worker) must produce the SAME weights and metrics as 1 worker, bit for
bit. Small fixture so two worker processes plus the parent stay under the 8 GB watchdog."""
...
def main():
    ...
    for k in ("f1_macro", "accuracy", "f1_macro_std", "global_f1_macro", "global_accuracy",
              "loss_s_client_mean", "loss_r_client_mean", "kl_s_client_mean"):
        assert abs(h1[k] - h2[k]) < 1e-12, (k, h1[k], h2[k])
    shutil.rmtree(tmp)
    print(f"\n1 worker vs 2 workers: max|dweight| = {dmax}, f1_macro {h1['f1_macro']:.6f} == "
          f"{h2['f1_macro']:.6f}, proxy {h1['global_f1_macro']:.6f} == {h2['global_f1_macro']:.6f}"
