...
NB = ROOT / "papers/pfedes-yi-2025/notebook/20c_probe/pfedes_20c_probe.ipynb"
CIDS = [0, 1, 2, 3]
...
def main():
    ...
    if gpu:
        ...
        assert cal["train_backend"] == "eager" and "eval_eager_folded_8192" in cal
        ...
        for f in ("history.csv", "clients.csv", "reports/manifest.json", "reports/y_true.u8.npy",
                  "weights/round_002.pt", "confusion/round_002.npy", "complete/round_002.done"):
            assert (d / f).exists(), f
    shutil.rmtree(tmp)
