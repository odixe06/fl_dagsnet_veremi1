...
resume gate (proj/ckpt.py, embedded byte-for-byte) will see it, up to the expected round?
    ...
    python scripts/gen_ckpt_probe.py --owner minhtrit06 --dataset minhtrit06/pfedes-100c-cos-ckpt-s2 \
        --run-name pfedes_100c_cos --expect-round 25 --out papers/pfedes-yi-2025/notebook/100c_ckpt_probe
...
ROOT = Path(__file__).resolve().parents[1] / "papers/pfedes-yi-2025"
...
shutil.rmtree("/kaggle/working/runs", ignore_errors=True)   # do not commit 4 GB of copies
